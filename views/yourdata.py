...
with u2:
    ...
    s1, s2 = st.columns(2)
    if s1.button("Breast Cancer sample", width="stretch", help="569 rows, 30 measurements, malignant or benign"):
        d = load_breast_cancer(as_frame=True)
        df = d.data.copy()
        df["diagnosis"] = d.target.map({0: "malignant", 1: "benign"})
        st.session_state.yd = ("Breast Cancer sample", df)
    if s2.button("Wine sample", width="stretch", help="178 rows, 13 measurements, three cultivars"):
...
with m1:
    ...
    for m in LIBRARY_MODELS:
        st.checkbox(f"{m}: {MODELS[m]['tag']}", key=f"yd_{m}")
with m2:
    st.markdown("**Baselines**")
    for m in BASELINE_MODELS:
        st.checkbox(f"{m}: {MODELS[m]['tag']}", key=f"yd_{m}")
selected = [m for m in MODELS if st.session_state.get(f"yd_{m}")]
if len(selected) < 2:
    st.info("Pick at least two models.")
    st.stop()

# ── step 3: run ──────────────────────────────────────────────────────
ui.step(3, "Run", "Every model sees the same folds; features are imputed, standardised and one-hot encoded inside each fold, never on the held-out part.")
...
def _fold(df, target, model_name, folds, seed, k):
    ...
    pipe = make_pipeline(preprocessor(numeric, categorical), build(model_name, defaults(model_name), 0))
    pipe.fit(X.iloc[train], y[train])
    return float((pipe.predict(X.iloc[test]) == y[test]).mean())
...
hosted_s = sum(SECONDS_PER_FOLD.get(m, 0.3) for m in selected) * folds * scale * HOSTED_SLOWDOWN
with st.container(border=True):
    st.markdown(f"**{len(selected)} models, {folds} folds, {len(y)} rows, {X.shape[1]} columns, {len(classes)} classes.** About **{max(1, round(hosted_s))} s** on the hosted app; a repeat of the same run is instant.")
        ...
        with st.status(f"Training {len(selected)} models on {folds} folds", expanded=True) as status:
            for m in selected:
                ...
                try:
                    ...
                    st.write(f"✓ {m}: {np.mean(scores):.3f} in {time.time() - t0:.1f} s")
...
for rank, n in enumerate(order, 1):
    ...
    rows.append({"rank": rank, "model": n, "accuracy": f"{s.mean():.3f} ± {s.std():.3f}", "gap (points)": f"{(s.mean() - ok_res[best].mean()) * 100:+.1f}", "p vs best": "" if np.isnan(p) else f"{p:.3f}", "reading": reading})
majority = float(np.bincount(y).max() / len(y))
ui.verdict(
    f"Result on {r_name}, predicting {r_target}, {r_folds} folds",
    f"{best} scored highest: {ok_res[best].mean():.3f} ± {ok_res[best].std():.3f}",
    (f"Within fold noise of it: <b>{', '.join(within)}</b>. " if within else "")
    + (f"Measurably behind: <b>{', '.join(behind)}</b>. " if behind else "")
    + f"Always predicting the commonest class would score {majority:.3f}; that is the floor every model has to clear.",
)
if failed:
...
fig.add_vline(x=majority, line=dict(color="rgba(21,32,43,0.4)", dash="dot"), annotation_text="majority class", annotation_position="top")
lo = max(0.0, min(min(ok_res[n].mean() - ok_res[n].std() for n in order), majority) - 0.05)
fig.update_layout(height=50 + 34 * len(order), margin=dict(t=30, b=10, l=10, r=20), xaxis=dict(range=[lo, 1.02], title="accuracy, mean ± sd over folds"), yaxis=dict(autorange="reversed"), plot_bgcolor="white", paper_bgcolor="white")
st.plotly_chart(fig, config={"displayModeBar": False})

# ── step 4: look inside ──────────────────────────────────────────────
ui.step(4, "Look inside the soft tree", "Fitted once on all the rows. The rules, one row explained, and the file to take away.")
if "Soft Decision Tree" not in r_models:
    st.info("Tick **Soft Decision Tree** in step 2 and run again to see rules and explanations.")
else:
    with st.spinner("Fitting the soft tree on all rows..."):
        pipe = _fit_all(df, target, "Soft Decision Tree")
    pre, tree = pipe.named_steps["columntransformer"], pipe.named_steps["softdecisiontree"]
    ...
    with st.expander(f"The tree as rules ({rules.count(chr(10)) + 1} lines; class numbers are {', '.join(f'{i} = {c}' for i, c in enumerate(classes))})"):
    ...
    d1, d2 = st.columns(2)
    d1.download_button("Download model.json", tree.to_numpy(feature_names=fnames).to_json(), file_name="neural-trees-model.json", mime="application/json", width="stretch")
    d1.caption("Expects the preprocessed features (the same imputation, scaling and one-hot encoding); the code on the right shows exactly that preprocessing.")
    with d2:
        st.code(code_snippet(target, numeric, categorical, "SoftDecisionTree(depth=4, max_epochs=40, random_state=0)"), language="python")
