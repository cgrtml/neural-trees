...
from neural_trees import (SoftDecisionTree, GALNetwork, OmnivariateDecisionTree,
...
def veri(ad):
    if ad == "Iris":   X, y = load_iris(return_X_y=True)
    elif ad == "Wine": X, y = load_wine(return_X_y=True)
    elif ad == "Cancer": X, y = load_breast_cancer(return_X_y=True)
    elif ad == "Digits": X, y = load_digits(return_X_y=True)
    elif ad == "Sentetik-800x20":
        X, y = make_classification(n_samples=800, n_features=20, n_informative=6,
            n_redundant=0, flip_y=0.05, class_sep=0.9, random_state=0)
    elif ad == "Sentetik-2000x50":
        X, y = make_classification(n_samples=2000, n_features=50, n_informative=12,
            n_redundant=8, flip_y=0.05, class_sep=0.8, n_classes=4,
...
def cv(yap, ad, olcu=None):
    ...
    for s in TOHUM:
        for tr, te in StratifiedKFold(5, shuffle=True, random_state=s).split(X, y):
            ...
            if olcu: ek.append(olcu(e))
    return (np.mean(acc), np.std(acc, ddof=1),
            (np.mean(ek), np.std(ek, ddof=1)) if ek else None)

def yaz(baslik, satirlar):
    print(f"\n### {baslik}")
    for s in satirlar: print("  " + s)
    sys.stdout.flush()

# ── 1 · JITTER · ana bulgu ────────────────────────────────────────────
orij = sdt_mod._SoftTreeModule.deepen
def jitterli(j):
    return lambda self, nc, jitter=0.0, _j=j: orij(self, nc, _j)
...
for ad in ("Iris", "Wine", "Digits"):
    ...
    for j in (0.0, 0.05, 0.2, 0.5, None):
        t0 = time.time()
        if j is None:
            yap = lambda s: SoftDecisionTree(depth=4, max_epochs=150, random_state=s)
        else:
            sdt_mod._SoftTreeModule.deepen = jitterli(j)
            yap = lambda s: SoftDecisionTree(depth=4, max_epochs=150,
...
class Buyuyen(ClassifierMixin, BaseEstimator):
    ...
    def __init__(self, jitter=0.2, depth=4, max_epochs=150, random_state=0):
        self.jitter=jitter; self.depth=depth
        self.max_epochs=max_epochs; self.random_state=random_state
    def fit(self, X, y):
        ...
        return self
    def predict(self, X): return self.ic_.predict(X)

SONUC["jitter_test"] = {}
for ad in ("Iris", "Wine"):
    X, y = veri(ad); X = StandardScaler().fit_transform(X)
    r = combined_5x2cv_f_test(Buyuyen(jitter=0.0), Buyuyen(jitter=0.2), X, y, random_state=0)
    SONUC["jitter_test"][ad] = [round(float(r.statistic),3), round(float(r.p_value),5)]
    yaz(f"5x2cv F · jitter 0 vs 0.2 · {ad}", [f"F={r.statistic:.3f}  p={r.p_value:.5f}"])
...
for ad in ("Iris", "Wine", "Digits"):
    ...
    for init in ("random", "residual"):
        m, sd, (bm, bsd) = cv(
            ...
            ad, olcu=lambda e: e.n_hidden_final_)
        SONUC["gal"].setdefault(ad, {})[init] = [round(m,4), round(sd,4), round(bm,2), round(bsd,2)]
        satir.append(f"{init:9s} acc {m:.3f} ± {sd:.3f}   birim {bm:.1f} ± {bsd:.1f}")
    yaz(f"GAL · {ad}", satir)

for ad in ("Iris", "Wine"):
    X, y = veri(ad); X = StandardScaler().fit_transform(X)
    r = combined_5x2cv_f_test(GALNetwork(max_epochs=150, growth_init="random", random_state=0),
                              GALNetwork(max_epochs=150, growth_init="residual", random_state=0),
                              X, y, random_state=0)
    SONUC.setdefault("gal_test", {})[ad] = [round(float(r.statistic),3), round(float(r.p_value),5)]
    yaz(f"5x2cv F · GAL random vs residual · {ad}", [f"F={r.statistic:.3f}  p={r.p_value:.5f}"])
...
for ad in ("Sentetik-800x20", "Sentetik-2000x50"):
    ...
    for g in ("none", "per_leaf"):
        m, sd, (bm, bsd) = cv(
            lambda s, gg=g: SoftDecisionTree(depth=6, max_epochs=180, growth=gg, random_state=s),
            ad, olcu=lambda e: len(e.get_split_weights()))
        SONUC["per_leaf"].setdefault(ad, {})[g] = [round(m,4), round(sd,4), round(bm,2), round(bsd,2)]
...
def dugum(e):
    ...
    def w(nd):
        n[0] += 1
        if nd.left is not None: w(nd.left); w(nd.right)
    w(e.root_); return n[0]
...
for ad in ("Cancer", "Digits"):
    ...
    for sec in ("accuracy", "test"):
        m, sd, (bm, bsd) = cv(
            ...
            ad, olcu=dugum)
        SONUC["omni"].setdefault(ad, {})[sec] = [round(m,4), round(sd,4), round(bm,2), round(bsd,2)]
        satir.append(f"{sec:9s} acc {m:.3f} ± {sd:.3f}   düğüm {bm:.1f} ± {bsd:.1f}")
    yaz(f"OMNIVARIATE · {ad}", satir)

import pathlib
pathlib.Path("paper/arxiv/olcum/sonuc.json").write_text(json.dumps(SONUC, indent=2, ensure_ascii=False))
