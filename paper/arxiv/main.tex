...
trains. We measured four such decisions in tree structured and constructive
...
One cannot work at all, and we prove why. Deepen a soft decision tree so that its
...
local question with a principled criterion. We implemented four of them, each
against the non growing baseline it is supposed to improve on, under one fixed
protocol.

The result we did not expect is how little the sensible sounding arguments
...
We use \emph{grow} for any decision that adds structure while the model trains.
...
built. It is a construction decision rather than a capacity decision, and we
...
library of these models with a PyTorch \cite{paszke2019pytorch} backend. We
...
negative outcomes left in. The negative ones have been more useful to us than
...
and folds are used for every arm of a comparison. We report the mean and, after
...
difference needs a decision rather than a description we use the combined
...
\texttt{neural-trees} v0.6.1 \cite{temel2026neuraltrees}.
...
we measure below. Asymmetric noise during training is another: the subtree
...
Floating point arithmetic does not supply such a perturbation here, and we
...
repair the operation without noise, and we have not tried it.
\end{remark}
...
Because this is the paper's central claim we test it rather than only describing
...
The Iris result sits just above the conventional threshold and we are not going
...
The size effect is consistent and the accuracy effect is not, and we want to be
...
One implementation detail cost us a day. The candidate objective involves a
...
We take the honest reading to be that the rule buys sparsity reliably and buys
...
does at each node. We kept the depth fixed across datasets rather than tuning it,
...
change sign. Those are two different questions and we can only answer the first
one.
...
is one line in a test. In our case it was the difference between a growth
...
The standard deviations we report describe fold to fold spread over fifteen
non independent fits. They are not confidence intervals and should not be read as
such. Where we needed to decide rather than describe we used the combined
$5\times2$cv $F$ test, which is reported alongside.

We did not try the obvious repair suggested by Remark~\ref{rem:asimetri}, which
...
perturbing the children at random. That is the experiment we would run next.
...
check and we now check it in a test.
...
\noindent The exact version used here is v0.6.1, archived at

\begin{center}
\url{https://doi.org/10.5281/zenodo.22718898}
\end{center}
